...
def register_provider_init_gate_for_agent(app: Any, agent_name: str) -> bool:
    ...
    builder = _GATE_BUILDERS[provider]
    try:
        gate = builder(app=app, agent_name=agent_name, runtime=runtime)
    except Exception as exc:
        logger.warning(
            "init_gate_register: action=build_failed agent=%s provider=%s reason=%r",
            agent_name, provider, exc,
        )
        return False
    ...
    logger.info(
        "init_gate_register: action=registered agent=%s provider=%s pane=%s",
        agent_name, provider, pane_id,
    )
    return True
...
def _build_tmux_run_str(runtime):
    """Adapter: TmuxBackend._tmux_run → ``(args) -> str`` for probes."""
    ...
    return _tmux_run_str


def _build_gate_with_probe(*, app: Any, agent_name: str, provider: str, probe) -> InitGate:
    """Wrap a probe in an InitGate, sharing per-provider env config + capture."""
    runtime_dir = app.paths.agent_provider_runtime_dir(agent_name, provider)
    ...
    gate_kwargs = load_init_gate_env(provider)
    return InitGate(
        probe=probe,
        provider=provider,
...
def _build_gemini_gate(*, app: Any, agent_name: str, runtime) -> InitGate:
    ...
    probe = GeminiInitProbe(
        pane_id=runtime.pane_id,
        tmux_run_fn=_build_tmux_run_str(runtime),
    )
    return _build_gate_with_probe(
        app=app, agent_name=agent_name, provider="gemini", probe=probe,
    )


def _build_claude_gate(*, app: Any, agent_name: str, runtime) -> InitGate:
    """Construct InitGate(probe=ClaudeInitProbe, ...) for one Claude agent."""
    from provider_backends.claude.init_probe import ClaudeInitProbe

    probe = ClaudeInitProbe(
        pane_id=runtime.pane_id,
        tmux_run_fn=_build_tmux_run_str(runtime),
    )
    return _build_gate_with_probe(
        app=app, agent_name=agent_name, provider="claude", probe=probe,
    )


# Provider → builder dispatch table. Add a new provider by writing a
# probe + builder, then adding the entry here.
# NOTE: codex is intentionally absent — its per-agent bridge process
# owns its own InitGate (see provider_backends.codex.bridge_runtime.
# runtime_state.build_bridge_runtime_state). Adding it here would
# double-register and tick the gate from two places.
_GATE_BUILDERS = {
    "gemini": _build_gemini_gate,
    "claude": _build_claude_gate,
}
